...
/// Return true when a menu item needs marquee animation in the ROM list.
...
/// Return the inclusive-start/exclusive-end y-range for a menu item slot.
...
/// Return the text-only window for a menu item slot.
...
/// Return the inclusive-start/exclusive-end y-range for the loading progress bar.
...
fn write_marquee_text_row(
    ...
    write_text_window_row(
        row, text, x_start, x_end, glyph_row, scale, scroll_px, true, fg, bg,
    );
}

fn write_truncated_text_row(
    row: &mut [u8; 480],
    text: &[u8],
    x_start: usize,
    x_end: usize,
...
fn write_text_window_row(
    ...
    let gap_w = MARQUEE_GAP_CHARS * CHAR_W * scale;
    let period_w = text_w + if wrap { gap_w } else { 0 };

    for px in x0..x1 {
        let source_px = px - x0 + scroll_px;
        let source_px = if wrap {
            source_px % period_w
        } else {
            source_px
        };
...
fn text_pixel_lit(text: &[u8], glyph_row: usize, scale: usize, source_px: usize) -> bool {
    let char_screen_w = CHAR_W * scale;
    ...
    let bitmap = font::glyph_row(ch, glyph_row);
    // Keep the existing LSB-first convention used by write_text_row.
    (bitmap >> glyph_x) & 1 != 0
}

fn text_screen_width(text: &[u8], scale: usize) -> usize {
