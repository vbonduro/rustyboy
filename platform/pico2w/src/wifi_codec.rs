...
pub fn url_decode_into<const N: usize>(src: &str, dst: &mut heapless::String<N>) {
    ...
    let flush_utf8 = |buf: &[u8; 4], len: usize, dst: &mut heapless::String<N>| {
        if len == 0 {
            return;
        }
        ...
        if b == b'+' {
            ...
            if let (Some(hi), Some(lo)) = (hex_nibble(bytes[i + 1]), hex_nibble(bytes[i + 2])) {
                ...
                if decoded < 0x80 {
                    ...
                    flush_utf8(&utf8_buf, utf8_len, dst);
                    utf8_len = 0;
                    // Safe: decoded is ASCII.
                    dst.push(decoded as char).ok();
                } else {
...
// ---------------------------------------------------------------------------
// DHCP server response builder (captive portal)
// ---------------------------------------------------------------------------
//
...
const DHCP_COOKIE_OFFSET: usize = 236;
/// Smallest message we will parse: fixed header + magic cookie.
const DHCP_MIN_LEN: usize = DHCP_COOKIE_OFFSET + 4; // 240
