...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/api/battery-saves/:rom_name",
            get(get_battery_save).put(put_battery_save),
        )
        .route("/api/save-states", get(list_roms_with_saves))
        .route(
            "/api/save-states/:rom_name",
            get(list_save_states).post(post_save_state),
        )
        .route(
            "/api/save-states/:rom_name/latest",
            get(get_latest_save_state),
        )
...
async fn inject_auth_extensions(
    ...
    req.extensions_mut().insert(DbExt(state.db.clone()));
    next.run(req).await
}
...
async fn api_me(auth: AuthUser, State(state): State<Arc<AppState>>) -> impl IntoResponse {
...
async fn delete_save_state(
    ...
    match state.db.get_save_state(&id).await {
        Ok(Some(s)) if s.user_id == auth.user_id => match state.db.delete_save_state(&id).await {
            Ok(_) => StatusCode::NO_CONTENT.into_response(),
            Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        },
        Ok(Some(_)) => StatusCode::FORBIDDEN.into_response(),
