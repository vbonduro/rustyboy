...
use rustyboy_web_server::{auth::OAuthConfig, build_router, db_connect, AppState};
use std::sync::Arc;
...
async fn test_get_save_state_data() {
    ...
    let post_res = app
        ...
        .unwrap();
    assert_eq!(post_res.status(), StatusCode::CREATED);
    let post_body = axum::body::to_bytes(post_res.into_body(), usize::MAX)
        .await
        .unwrap();
    ...
    let get_res = app
        ...
        .unwrap();
    assert_eq!(get_res.status(), StatusCode::OK);
    let data = axum::body::to_bytes(get_res.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_delete_save_state() {
    ...
    let del_res = app
        ...
        .oneshot(
            Request::builder()
                .method("DELETE")
                .uri(format!("/api/save-states/by-id/{id}"))
                .header("cookie", &cookie)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn test_get_latest_save_state() {
    ...
    let post2 = app
        ...
        .await
        .unwrap();
    let post2_body = axum::body::to_bytes(post2.into_body(), usize::MAX)
        .await
        .unwrap();
    ...
    let latest_body = axum::body::to_bytes(latest_res.into_body(), usize::MAX)
        .await
        .unwrap();
...
async fn test_list_roms_with_saves() {
    ...
    for rom in &["tetris.gb", "mario.gb"] {
        app.clone()
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri(format!("/api/save-states/{rom}"))
                    .header("cookie", &cookie)
                    .header("content-type", "application/octet-stream")
                    .body(Body::from(vec![1u8]))
                    .unwrap(),
    ...
    let names: Vec<&str> = roms
        .iter()
        .map(|r| r["rom_name"].as_str().unwrap())
        .collect();
