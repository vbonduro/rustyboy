...
async fn session_cookie_is_samesite_strict() {
    ...
    assert!(
        ...
        "SameSite=Lax must not be present, got: {cookie}"
    );
    let _ = cookie_str;
}
...
async fn logout_clear_cookie_is_samesite_strict() {
    ...
    assert!(
        ...
        "logout clear-cookie must be SameSite=Strict, got: {set_cookie}"
    );
}
...
async fn token_revoked_after_logout() {
    ...
    assert_eq!(
        res.status(),
        StatusCode::OK,
        "/api/me must work before logout"
    );
    ...
    assert_eq!(
        ...
        "old token must be rejected after logout"
    );
}
...
async fn logout_rejects_cross_origin() {
    ...
    assert_eq!(
        ...
        "cross-origin logout must be rejected"
    );
}
