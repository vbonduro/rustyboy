...
fn run_rom(path: &str) -> String {
    ...
    let rom_data = std::fs::read(&full_path)
        .unwrap_or_else(|_| panic!("ROM not found: {} — run tests from repo root", full_path.display()));

    let memory = Box::new(GameBoyMemory::with_rom(rom_data));
    let decoder = Box::new(OpCodeDecoder::new());
    let mut cpu = Sm83::new(memory, decoder).with_registers(rustyboy_core::cpu::registers::Registers {
        pc: 0x0100,
        sp: 0xFFFE,
        ..Default::default()
    });
