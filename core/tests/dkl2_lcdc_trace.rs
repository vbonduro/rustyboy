...
fn build_cpu() -> GameBoy {
    let rom_path =
        "/home/vbonduro/roms/extracted/Donkey Kong Land 2 (USA, Europe) (SGB Enhanced).gb";
...
fn dump_oam_summary(cpu: &GameBoy) -> usize {
    ...
    for i in 0..40 {
        ...
        if y != 0 && y < 160 {
            eprintln!(
                "  OAM[{:2}]: scr({:3},{:3}) tile=0x{:02X} pal={} bg_pri={}",
                i,
                x as i16 - 8,
                y as i16 - 16,
                tile,
...
fn trace_dkl2_stat_interrupt_ffeb() {
    ...
    for f in 0..710u32 {
        if f == 120 {
            cpu.set_button(Button::Start, true);
        }
        if f == 130 {
            cpu.set_button(Button::Start, false);
        }
        if f == 350 {
            cpu.set_button(Button::A, true);
        }
        if f == 360 {
            cpu.set_button(Button::A, false);
        }
        if f == 450 {
            cpu.set_button(Button::Start, true);
        }
    ...
    for frame_offset in 0..5u32 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if (cur_if & 0x01) != 0 && (prev_if & 0x01) == 0 {
                eprintln!(
                    "  F+{} VBLK INT PENDING: LY={:3} LCDC=0x{:02X}(OBJ={}) FFEB=0x{:02X}",
                    frame_offset,
                    cur_ly,
                    cur_lcdc,
                    (cur_lcdc >> 1) & 1,
            ...
            if cur_lcdc != prev_lcdc {
                eprintln!(
                    "  F+{} LCDC CHANGE: LY={:3} 0x{:02X}->0x{:02X} OBJ {}->{}  FFEB=0x{:02X}",
                    frame_offset,
                    cur_ly,
                    prev_lcdc,
                    cur_lcdc,
            ...
            if cur_ly != prev_ly && (cur_ly == 0 || cur_ly == 128 || cur_ly == 144) {
                ...
                eprintln!(
                    "  F+{} LY={:3}: LCDC=0x{:02X} STAT=0x{:02X} LYC={:3} FFEB=0x{:02X}",
...
fn trace_dkl2_ffeb_timeline() {
    ...
    for frame_offset in 0..10u32 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if ffeb != prev_ffeb {
                eprintln!(
                    "  F+{} FFEB: 0x{:02X}->0x{:02X}  LY={:3} LCDC=0x{:02X} PC=0x{:04X}",
            ...
            if ffe6 != prev_ffe6 {
                eprintln!(
                    "  F+{} FFE6: 0x{:02X}->0x{:02X}  LY={:3} PC=0x{:04X}",
            ...
            if ffee != prev_ffee {
                eprintln!(
                    "  F+{} FFEE: 0x{:02X}->0x{:02X}  LY={:3} PC=0x{:04X}",
    ...
    eprintln!("\nFinal HRAM state:");
    for addr in [
        0xFFE6u16, 0xFFE7, 0xFFE8, 0xFFE9, 0xFFEA, 0xFFEB, 0xFFEC, 0xFFED, 0xFFEE, 0xFFEF,
    ] {
        eprintln!(
            "  0x{:04X} = 0x{:02X}",
            addr,
    ...
    for frame_offset in 0..3u32 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if (in_vblank_isr || in_stat_region || in_14720_region) && !logged_pcs.contains(&pc) {
                eprintln!(
                    "  F+{} PC=0x{:04X} LY={:3} LCDC=0x{:02X}(OBJ={}) IF=0x{:02X} FFEB=0x{:02X}",
                    frame_offset,
                    pc,
                    ly,
                    lcdc,
            ...
            if (cur_if & 0x03) != (prev_if & 0x03) {
                eprintln!(
                    "  F+{} IF: 0x{:02X}->0x{:02X}  LY={:3} PC=0x{:04X} FFEB=0x{:02X}",
...
fn trace_dkl2_level_entry_lyc() {
    ...
    for _ in 0..2 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if lyc != prev_lyc_t {
                eprintln!(
                    "  LYC: {}->{}  LY={} PC=0x{:04X} FFE6=0x{:02X} FFEB=0x{:02X}",
            ...
            if ffe6 != prev_ffe6_t {
                eprintln!(
                    "  FFE6: 0x{:02X}->0x{:02X}  LY={} PC=0x{:04X} FFEB=0x{:02X}",
            ...
            if ffeb != prev_ffeb_t {
                eprintln!(
                    "  FFEB: 0x{:02X}->0x{:02X}  LY={} PC=0x{:04X}",
...
fn trace_dkl2_ime_timing() {
    ...
    for frame_offset in 0..3u32 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if cur_ime != prev_ime {
                eprintln!(
                    "  F+{} IME: {}->{}  LY={:3} PC=0x{:04X} IF=0x{:02X} LCDC=0x{:02X}(OBJ={})",
                    frame_offset,
            ...
            if ly != prev_ly && (ly == 0 || ly == 128 || ly == 144) {
                eprintln!(
                    "  F+{} LY={:3}: IME={} IF=0x{:02X} PC=0x{:04X} LCDC=0x{:02X}(OBJ={})",
                    frame_offset,
                    ly,
                    if cur_ime { "ON" } else { "off" },
                    cur_if,
...
fn trace_dkl2_ie_level_entry() {
    ...
    for f in 648u32..700 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if ie != prev_ie {
                eprintln!(
                    "  F{:04} IE:   0x{:02X}->0x{:02X}  LY={:3} LYC={:3} LCDC=0x{:02X} PC=0x{:04X}",
            ...
            if lcdc != prev_lcdc {
                eprintln!(
                    "  F{:04} LCDC: 0x{:02X}->0x{:02X} OBJ {}->{}  LY={:3} IE=0x{:02X} PC=0x{:04X}",
                    f,
                    prev_lcdc,
                    lcdc,
                    if prev_lcdc & 2 != 0 { "ON " } else { "OFF" },
            ...
            if if_ != prev_if {
                eprintln!(
                    "  F{:04} IF:   0x{:02X}->0x{:02X}  LY={:3} LCDC=0x{:02X} PC=0x{:04X}",
            ...
            if stat != prev_stat {
                eprintln!(
                    "  F{:04} STAT: 0x{:02X}->0x{:02X}  LY={:3} PC=0x{:04X}",
...
fn trace_dkl2_level_lcdc() {
    ...
    for f in 0..1000u32 {
        ...
        if f >= 710 && f < 800 {
            cpu.set_button(Button::Right, true);
        }
        if f == 800 {
            cpu.set_button(Button::Right, false);
        }
    ...
    let lyc = cpu.read_memory(LYC_ADDR).unwrap_or(0);
    eprintln!(
        "LCDC=0x{:02X} OBJ={} BGP=0x{:02X} STAT=0x{:02X} LYC={}",
        lcdc,
        (lcdc >> 1) & 1,
        bgp,
        stat,
    ...
    for (ly, old, new) in &changes {
        eprintln!(
            "  LY={:3}: 0x{:02X}→0x{:02X}  OBJ {}→{}",
            ly,
            old,
            new,
            if old & 2 != 0 { "ON " } else { "OFF" },
            if new & 2 != 0 { "ON" } else { "OFF" }
        );
    }
    ...
    for row in (0..144usize).step_by(2) {
        let row_str: String = fb[row * 160..(row + 1) * 160]
            .iter()
            .map(|&p| match p {
                0 => ' ',
                1 => '.',
    ...
    for frame_offset in 0..5u32 {
        ...
        while cpu.cycle_counter().wrapping_sub(start) < CYCLES_PER_FRAME as u64 {
            ...
            if pc == 0x35C2 && !in_loop {
                ...
                loop_b = b;
                eprintln!(
                    "  F+{} ENTER loop 0x35C2: B={:3} LY={:3} IF={:02X} IME={}",
            ...
            if pc == 0x35C6 && in_loop {
                in_loop = false;
                eprintln!(
                    "  F+{} EXIT  loop -> EI: B={:3} LY={:3} IF={:02X} IME={}",
                    frame_offset, b, ly, if_, ime
                );
            }
            ...
            if in_loop && b != loop_b {
                eprintln!(
                    "  F+{} LOOP B changed: {}->{}  LY={:3} PC=0x{:04X}",
