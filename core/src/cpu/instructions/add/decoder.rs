...
impl Decoder for Add16Decoder {
    fn decode(&self, opcode: u8) -> Result<Box<dyn OpCode>, Error> {
        match opcode {
            0x09 => Ok(Box::new(Add16 {
                operand: Operand::Register16(Register16::BC),
                cycles: 8,
            })),
            0x19 => Ok(Box::new(Add16 {
                operand: Operand::Register16(Register16::DE),
                cycles: 8,
            })),
            0x29 => Ok(Box::new(Add16 {
                operand: Operand::Register16(Register16::HL),
                cycles: 8,
            })),
            0x39 => Ok(Box::new(Add16 {
                operand: Operand::Register16(Register16::SP),
                cycles: 8,
            })),
            _ => Err(Error::InvalidOpcode(opcode)),
        }
...
impl Decoder for AddSP16Decoder {
    fn decode(&self, opcode: u8) -> Result<Box<dyn OpCode>, Error> {
        match opcode {
            0xE8 => Ok(Box::new(AddSP16 {
                operand: Operand::ImmSigned8,
                cycles: 16,
            })),
            _ => Err(Error::InvalidOpcode(opcode)),
        }
